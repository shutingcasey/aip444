          ...
          files: {
            ...
            items: {
              ...
              properties: {
                ...
                ref: {
                  type: 'string',
                  description: 'The branch name, commit, or tag (e.g., main).',
                },
              },
...
async function callOpenRouter(
  ...
  for (let iteration = 1; iteration <= maxIterations; iteration++) {
    ...
    if (!assistantMessage) {
      throw new Error('No assistant message returned from OpenRouter');
    }
    ...
    console.log(`🛠️ Tool calls requested: ${assistantMessage.tool_calls.length}`);
    ...
    for (const toolCall of assistantMessage.tool_calls) {
      ...
      console.log(`📦 Tool arguments: ${rawArgs}`);
      ...
      try {
        ...
        toolResult = `Error while running tool "${functionName}": ${String(error)}`;
      }
  ...
  return 'Max tool-calling iterations reached before the model produced a final answer.';
}
